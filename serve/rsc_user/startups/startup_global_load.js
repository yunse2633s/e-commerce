...
module.exports = function () {
    var fileArr = fs.readdirSync(__dirname.replace('startups', 'libs'));
    ...
    fileArr = fs.readdirSync(__dirname.replace('startups', 'configs'));
    ...
    fileArr = fs.readdirSync(__dirname.replace('startups', 'services'));
    for (var k = 0; k < fileArr.length; k++) {
        fileName = fileArr[k];
        fileName = fileName.split('.')[0];
        if (fileName) global[fileName] = require('../services/' + fileName);
    }
