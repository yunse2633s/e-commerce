...
var DB = require('../dbs/db_base')('Tip');
...
exports.getTime = function(data, flag, callback){
  async.waterfall([function(cb){
      DB.getOne({find: data}, cb);
  }, function(timeRes, cb){
      if(!timeRes){
          data.update_time = new Date();
          DB.add(data, cb);
      }else if(timeRes && flag){
          timeRes.update_time = new Date();
          timeRes.save(cb);
      }else{
          cb(null, timeRes, null);
      }
  }, function(timeRes, count, cb){
      cb(null, timeRes)
  }], callback);
};
