...
module.exports = function () {
    ...
    app.all("*", function (req, res, next) {
        ...
        //云信使用charset, CurTime, MD5, CheckSum
        res.header("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Content-Length, Accept, User-Agent, x-access-token, version, charset, CurTime, MD5, CheckSum, package-name");
    ...
    //第三方业务逻辑
    fileArr = fs.readdirSync(__dirname.replace('startups', 'routes/tps'));
    for (var i = 0; i < fileArr.length; i++) {
        fileName1 = fileArr[i];                     //包括api名,包括扩展名      api_truck.js
        fileName2 = fileName1.split('.')[0];        //包括api名,不包括扩展名    api_truck
        fileName3 = fileName2.split('api_')[1];     //不包括api名,            truck
        if (fileName2 && fileName3) {
            app.use('/api/tps/' + fileName3, require('../routes/tps/' + fileName2)());
        }
    }
